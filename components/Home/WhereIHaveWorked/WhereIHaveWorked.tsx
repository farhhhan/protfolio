...
import MalabarSoft from "./Descriptions/MalabarSoft";
import Growblic from "./Descriptions/Growblic";
...
export default function WhereIHaveWorked() {
  ...
  const [DescriptionJob, setDescriptionJob] = React.useState("Growblic");
  return (
...
const CompaniesBar = props => {
  ...
  return (
    ...
    >
      {/* // ? left bar Holder */}
      <div
        className=" hidden md:block bg-gray-500 relative h-0.5 w-34 md:h-[100px] translate-y-1 md:w-0.5  
      ...
      <div className="flex flex-col md:order-2 order-1 space-y-1 pl-8 md:pl-0 ">
        <div className="flex flex-row md:flex-col">
          <CompanyButton
            ...
            CompanyName="Growblic PVT LTD"
            BarPosition={-10}
            BarAvobePosition={0}
            DescriptionJob="Growblic"
            CompanyNameBackgroundColorGreen={[true, false]}
            setDescriptionJob={props.setDescriptionJob}
          />
          <CompanyButton
            ButtonOrderOfcompanyNameBackgroundColorGreen={1}
            CompanyName="MalabarSoft LLC"
            BarPosition={36}
            BarAvobePosition={160}
            DescriptionJob="MalabarSoft"
            CompanyNameBackgroundColorGreen={[false, true]}
            setDescriptionJob={props.setDescriptionJob}
          />
        </div>
        <div className="block md:hidden h-0.5 rounded bg-gray-500">
          <motion.div animate={{ x: barAbovePosition }} className="w-[160px] h-0.5 rounded bg-AAsecondary"></motion.div>
        </div>
      </div>
    </div>
  );
